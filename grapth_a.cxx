...
template <typename T>
grapth_a<T>::grapth_a(std::vector<std::vector<T> > graf) { this->grapth = graf; }
...
std::vector<bool>	grapth_a<T>::find_depth(int x, std::vector<bool> visit)
{
	...
	for (int i = 0; i < grapth[x].size(); i++)
	{
		if (grapth[x][i] != 0)
		{
			if (visit[i] == false)
			{
				std::cout<< "From " << x + 1 << " to " << i + 1 << std::endl;
	...
	while (!q.empty())
	{
		...
		for (int i = 0; i < grapth[v].size(); ++i)
		{
			if (grapth[v][i] != 0)
			{
				if (visit[i] == false)
				{
					std::cout<< "From " << v + 1 << " to " << i + 1 << std::endl;
					q.push(i);
					visit[i] = true;
				}
			}
...
void				grapth_a<T>::Dijkstra(int s)
{
	...
	std::cout << "Result for s = " << s << std::endl;
...
void				grapth_a<T>::Floyd(void)
{
	...
	std::cout << "Result for Floyd" << std::endl;
	if (neg == true)
		std::cout << "There is at least one negative weight cycle" << std::endl;
	else
	{
		for (int i = 0; i < D.size(); i++)
		{
...
void				grapth_a<T>::Prim(void)
{
	...
	std::cout << "Result Prim" << std::endl;
	for (int i = 0; i < P.size(); ++i)
	{
		if (P[i] != 0)
		{
			std::cout << "v = " << i + 1 << " Parent[" << i + 1 << "] = " << P[i] << std::endl;
			edge.first = i + 1;
			edge.second = P[i];
			res.push_back(edge);
		}
...
void				grapth_a<T>::Kruskal(void)
{
	...
	std::cout<< "Result for Kruskal" << std::endl;
